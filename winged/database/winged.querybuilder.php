...
class QueryBuilder
{
    ...
    public function into($table_name)
    {
        if (!CurrentDB::tableExists($table_name)) {
            CoreError::_die(__CLASS__, "Table " . $table_name . " no exists in database " . WingedConfig::$DBNAME, __FILE__, __LINE__);
        }
        ...
        if (count($from) > 0) {
            ...
            if (is_string($key[0]) && is_string($from[$key[0]]) && $this->from_arr == '') {
                if (!CurrentDB::tableExists(trim($from[$key[0]]))) {
                    CoreError::_die(__CLASS__, "Table " . trim($from[$key[0]]) . " no exists in database " . WingedConfig::$DBNAME, __FILE__, __LINE__);
                }
        ...
        if ($condition != '' && count($inner) > 0) {
            ...
            if (is_string($key[0]) && is_string($key[0])) {
                if (!CurrentDB::tableExists(trim($inner[$key[0]]))) {
                    CoreError::_die(__CLASS__, "Table " . trim($inner[$key[0]]) . " no exists in database " . WingedConfig::$DBNAME, __FILE__, __LINE__);
                }
        ...
        if ($condition != '' && count($args) > 0) {
            if ($condition == DbDict::BETWEEN) {
                if (count($args) == 3) {
                    ...
                    CoreError::push(__CLASS__, "Between requires exactly three args in array. Given: " . count($args), __FILE__, __LINE__);
                }
                ...
                if (is_string($key[0])) {
                    if (is_array($args[$key[0]])) {
                        ...
                        CoreError::push(__CLASS__, "" . $condition . " requires an array in associative args array. Given: " . gettype($args[$key[0]]), __FILE__, __LINE__);
                    }
                }
            } else {
                $key = array_keys($args);
