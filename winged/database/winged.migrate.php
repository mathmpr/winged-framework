    ...
    public function createTableIfNotExists($args = array(), $echo = false)
    {
        if (!empty($args)) {
            if (!array_key_exists('table_name', $args)) {
                $warn = CoreError::push(__CLASS__, 'Table name not specified in args array.', __FILE__, __LINE__);
                        ...
                        if ((!in_array(strtolower($key), $reserved) && is_array($value)) || ($value === 'safe' || $value === true)) {
                            ...
                            if (array_key_exists('fk', $value)) {
                                ...
                                if (array_key_exists('reference_to', $value) && $value['reference_to'] !== '') {
                                    ...
                                    $warn = CoreError::push(__CLASS__, '' . $key . ' field is set to FK , however no table was set as a reference.', __FILE__, __LINE__);
                                    winged_error_handler('8', $warn['error_description'], __FILE__, 'in class : ' . __LINE__, $warn['real_backtrace']);
                                    CoreError::display(__LINE__, __FILE__);
                                }
                                if (array_key_exists('reference_field', $value) && $value['reference_field'] !== '') {
                                    ...
                                    $warn = CoreError::push(__CLASS__, '' . $key . ' field is set to FK , however no field was set as a reference for table ' . $value['reference_to'] . '', __FILE__, __LINE__);
                                    winged_error_handler('8', $warn['error_description'], __FILE__, 'in class : ' . __LINE__, $warn['real_backtrace']);
                                    CoreError::display(__LINE__, __FILE__);
                                }
            ...
            $warn = CoreError::push(__CLASS__, 'Empty table args.', __FILE__, __LINE__);
            winged_error_handler('8', $warn['error_description'], __FILE__, 'in class : ' . __LINE__, $warn['real_backtrace']);
            CoreError::display(__LINE__, __FILE__);
        }
    }
