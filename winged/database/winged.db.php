...
class Database
{
    ...
    function __construct($class = false, $driver = false, $nickname = false)
    {
        ...
        if ($WCclass !== IS_MYSQLI && $WCclass !== IS_PDO) {
            $warn = CoreError::push(__CLASS__, "Class " . $WCclass . " not suported by Winged dadabase connections.", __FILE__, __LINE__);
        ...
        if (!in_array($WCdriver, $this->drivers)) {
            $warn = CoreError::push(__CLASS__, "Driver " . $WCdriver . " not suported by Winged dadabase connections.", __FILE__, __LINE__);
            winged_error_handler("8", $warn["error_description"], __FILE__, "in class : " . __LINE__, $warn["real_backtrace"]);
            CoreError::display(__LINE__, __FILE__);
        }

        if ($WCdriver !== DB_DRIVER_MYSQL && $WCclass === IS_MYSQLI) {
            CoreError::push(__CLASS__, "mysqli class don't suports driver " . WingedConfig::$DB_DRIVER . ". Please change the driver in ./config.php to DB_DRIVER_MYSQL ou change STD_DB_CLASS in ./config.php to IS_PDO", __FILE__, __LINE__);
    ...
    public function connect($args = [])
    {
        ...
        if (!$this->analyze_error()) {
            CoreError::push(__CLASS__, "Can't connect in database, please check the credentials in ./config.php", __FILE__, __LINE__);
            CoreError::push(__CLASS__, "Error: " . $this->db, __FILE__, __LINE__);
            CoreError::display(__LINE__, __FILE__);
        }
...
class PreparedMysqli
{
    ...
    public function execute($query = '', $args = [])
    {
        ...
        if ($this->bind_param($stmt, $args)) {
            ...
            try {
                ...
                CoreError::push(__CLASS__, "DB error: " . $error->getMessage(), __FILE__, __LINE__);
            }
    ...
    public function insert($query = '', $args = [])
    {
        $stmt = $this->refer->prepare($query);
        if ($stmt === false) {
            CoreError::push(__CLASS__, "DB error: can't prepare query - " . $query . ' : ' . $this->refer->error, __FILE__, __LINE__);
            return $this->refer->error;
        }
    ...
    public function count($query = '', $args = [])
    {
        ...
        $stmt = $this->refer->prepare($query);
        if ($stmt === false) {
            CoreError::push(__CLASS__, "DB error: can't prepare query - " . $query, __FILE__, __LINE__);
            return $this->refer->error;
        }
...
class NormalPDO
{
    ...
    public function execute($query = '')
    {
        $stmt = $this->refer->prepare($query);
        if ($stmt === false) {
            CoreError::push(__CLASS__, "DB error: can't prepare query - " . $query . ' : ' . $this->refer->errorInfo(), __FILE__, __LINE__);
