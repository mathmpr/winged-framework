...
class Controller
{
                    ...
                    if (class_exists($this->controller_name)) {
                        ...
                        if (method_exists($obj, 'beforeAction')) {
                            ...
                            if ($return !== null) {
                                ...
                                if (Winged::warning_exists() && WingedConfig::$CONTROLLER_DEBUG) {
                            ...
                            if (!empty($apply)) {
                                foreach ($apply as $key => $value) {
                                    if (!isset($this->method_args[$value->name])) {
                                        ...
                                        Winged::push_warning(__CLASS__, "Action '" . $method . "' requires parameter'" . $value->name . "'", true);
                                    } else {
                                        $to_call[] = $this->method_args[$value->name];
                                    }
                                }
                        ...
                        if (WingedConfig::$CONTROLLER_DEBUG) {
                            Winged::push_warning(__CLASS__, "Controller class '" . $this->controller_name . "' no exists in file '" . $this->controller_path . "'", true);
                        }
                    }
                }
            }
    ...
    public function loadModel($model = "", $root = false)
    {
        ...
        if (file_exists($path)) {
            ...
            Winged::push_warning(__CLASS__, "model in " . $path . " can't included because file not found.");
        }
    }
        ...
        if (file_exists($path) && !is_directory($path)) {
            ...
            if (Winged::$controller_debug) {
                if ($this->first_render) {
                    ...
                    $content = Winged::obGet();
                    if (!Winged::warning_exists()) {
                        Winged::obFinish();
                        echo '<html>';
    ...
    public function addJs($identifier, $string, $url = false)
    {
        if (file_exists($string) && !is_directory($string) && !$url) {
            if (array_key_exists($identifier, $this->js)) {
                Winged::push_warning(__CLASS__, "Index '" . $identifier . "' was doubled, js file '" . $this->js[$identifier]['string'] . "' never load.", true);
            }
            ...
            if (array_key_exists($identifier, $this->js)) {
                Winged::push_warning(__CLASS__, "Index '" . $identifier . "' was doubled, script '" . htmlspecialchars($this->js[$identifier]['string']) . "' never load.", true);
            }
            $this->js[$identifier] = array();
            $this->js[$identifier]['string'] = $string;
            ...
            if (array_key_exists($identifier, $this->js)) {
                Winged::push_warning(__CLASS__, "Index '" . $identifier . "' was doubled, script url '" . $this->js[$identifier]['string'] . "' never load.", true);
            }
            $this->js[$identifier] = array();
            $this->js[$identifier]['string'] = $string;
    ...
    public function addCss($identifier, $string, $url = false)
    {
        if (file_exists($string) && !is_directory($string) && !$url) {
            if (array_key_exists($identifier, $this->css)) {
                Winged::push_warning(__CLASS__, "Index '" . $identifier . "' was doubled, script file '" . $this->css[$identifier]['string'] . "' never load.", true);
            }
            ...
            if (array_key_exists($identifier, $this->css)) {
                Winged::push_warning(__CLASS__, "Index '" . $identifier . "' was doubled, css '" . htmlspecialchars($this->css[$identifier]['string']) . "' never load.", true);
            }
            $this->css[$identifier] = array();
            $this->css[$identifier]['string'] = $string;
            ...
            if (array_key_exists($identifier, $this->css)) {
                Winged::push_warning(__CLASS__, "Index '" . $identifier . "' was doubled, css url '" . $this->css[$identifier]['string'] . "' never load.", true);
            }
            $this->css[$identifier] = array();
            $this->css[$identifier]['string'] = $string;
