$(function () {
    ...
    $('.seo-length').each(function () {
        var inp = $(this);
        inp.closest('div').append($('<span class="char-count label label-primary">' + inp.attr('max-length') + '</span>').css({
            position: 'absolute',
            display: 'block',
            width: '50px',
            right: '17px',
        ...
        var tot = parseInt(inp.attr('max-length'));
        var current = tot - inp.val().length;
        ...
        inp.on('keyup', function () {
            var now = $(this);
            var charc = now.closest('div').find('.char-count');
            var tot = parseInt(now.attr('max-length'));
            var current = tot - now.val().length;
            var result = current * 100 / tot;
            if (current < 0) {
                current = 0;
            }
    ...
    $('#SeoPages_slug').on('keyup', function () {
        var self = $(this);
    ...
    $('#SeoPages_slug').on('blur', function () {
        var self = $(this);
        var val = self.val().toLowerCase();
        val = val.split('-');
        var nval = '';
    ...
    $('#SeoPages_page_title').on('keyup', function () {
        var self = $(this);
        var key = removeDiacritics($('#SeoPages_keywords').val()).toLowerCase();
        var title = removeDiacritics(self.val()).toLowerCase();

        var regex = new RegExp('\\b' + key + '\\b');

        if (title.search(regex) === -1) {
            self.closest('.find-prev').find('.error-msg').html('<b>O Título H1 não possui está palavra chave</b>');
        } else {
            self.closest('.find-prev').find('.error-msg').html('');
        }
    });

    $('#SeoPages_keywords').on('keyup', function () {
        var self = $(this);
        var title = removeDiacritics($('#SeoPages_page_title').val()).toLowerCase();
        var key = removeDiacritics(self.val()).toLowerCase();
