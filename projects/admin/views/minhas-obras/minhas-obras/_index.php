                        ...
                        <div class="datatable-scroll-wrap">
                            <table class="table table-bordered no-footer dataTable">
                                ...
                                <tr role="row">
                                    ...
                                    <th rowspan="1" colspan="1">Cidade
                                    </th>
                                    <th rowspan="1" colspan="1">Estado
                                    </th>
                                    <th class="sorting"
                                        onclick="window.open('<?= Admin::buildGetUrlSorting('sort_rua') ?>', '_self');"
                                        rowspan="1" colspan="1">Endereço
                                    </th>
                                    <th style="width: 50px;">Controles</th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php
                                /**
                                 * @var $models array | Obras[]
                                 * @var $model Obras
                                 */
                                foreach ($models as $model) {
                                    $cidade = (new Cidades())->select()->from(['C' => 'cidades'])->where(ELOQUENT_EQUAL, ['C.id_cidade' => $model->cidade])->one();
                                    $estado = (new Estados())->select()->from(['E' => 'estados'])->where(ELOQUENT_EQUAL, ['E.id_estado' => $model->estado])->one();
                                    ?>
                                    <tr role="row" class="even">
                                        <td><?= $model->nome ?></td>
                                        <td><?= $cidade->cidade ?></td>
                                        <td><?= $estado->estado ?></td>
                                        <td><?= $model->rua ?>, <?= $model->numero ?></td>
                                        <td style="width: 50px;">
                                            <a href="<?= \Winged\Winged::$page_surname ?>/visualizar/<?= $model->primaryKey() ?>"
                                               class="btn bg-primary-400" data-placement="top" data-popup="tooltip"
                                               title="" data-original-title="Vizualizar detalhes da obra">
                                                <i class="icon-eye"></i>
                                            </a>
                                        </td>
                                    </tr>
