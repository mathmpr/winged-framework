...
class Config
{
    ...
    public static function addRequiredJs(Controller $controller)
    {
        ...
        if ($response->ok()) {
            ...
            $controller->addJs('pagseguro.sessionId', '<script>var PagSeguroSessionId = \''. $response->id .'\';</script>');
        }
        $controller->addJs('pagseguro.directpayment', 'https://stc.' . self::getSandbox() . 'pagseguro.uol.com.br/pagseguro/api/v2/checkout/pagseguro.directpayment.js', [], true);
    }
